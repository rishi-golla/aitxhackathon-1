...
export default function RecentActivityCard() {
  ...
  return (
    <div className="w-full rounded-2xl bg-card shadow-card border border-white/5 p-5 hover:shadow-card-hover hover:border-cyan/20 transition-all duration-300 flex flex-col animate-slide-up flex-1 min-h-0" style={{ animationDelay: '400ms' }}>
      {/* Header */}
      <div className="flex items-start justify-between mb-4">
        <h3 className="text-white/80 text-sm font-medium">Recent Activity</h3>
      ...
      {/* Activity List */}
      <div className="flex-1 space-y-3.5 overflow-y-auto min-h-0 pr-2">
        {activities.map((activity, index) => (
          <div 
            key={activity.id} 
            className={`flex items-center gap-3 p-2.5 rounded-lg hover:bg-white/5 transition-all duration-200 ${
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[10px] text-white/50 leading-relaxed mb-0.5">{activity.title}</p>
              <p className="text-xs text-white/90 font-medium truncate">{activity.subtitle}</p>
            </div>
            <div 
      ...
      {/* Mark as Read Button */}
      <button className="mt-4 w-full py-2.5 rounded-lg bg-gradient-to-r from-cyan/10 to-accent/10 border border-cyan/30 text-cyan text-xs font-medium hover:from-cyan/20 hover:to-accent/20 hover:border-cyan/50 hover:shadow-glow transition-all duration-200">
        Mark as read
      </button>
    </div>
  )
