    ...
    try:
        ...
        with tarfile.open(local_path, "r") as tar:
            ...
            if mp4_member:
                ...
                tar.extract(mp4_member, path=output_dir)
                # Rename to a simple name
                extracted_path = os.path.join(output_dir, mp4_member.name)
                final_path = os.path.join(output_dir, "test_footage.mp4")
                
                if os.path.exists(final_path):
                    os.remove(final_path)
                os.rename(extracted_path, final_path)
                print(f"Success! Sample video saved as '{final_path}'")
            else:
